...
export class WeatherService {
  ...
  getWeatherByCity(city: string): Observable<WeatherInfo> {
    const apiUrl = `${API_URL}/data/2.5/weather?q=${city}&appid=${this.apiKey}&units=metric`;
    return this.http.get<WeatherInfo>(apiUrl);
  }

  getWeatherByCoordinates(latitude: number, longitude: number): Observable<WeatherInfo> {
    const apiUrl = `${API_URL}/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=${this.apiKey}&units=metric`;
    return this.http.get<WeatherInfo>(apiUrl);
  }
